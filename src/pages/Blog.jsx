    ...
    return (
      <div className="relative w-screen overflow-hidden text-black">
        ...
        <div className={ (currentSet=='0'? `right-0 `: `left-0 `)+`fixed bottom-8 flex justify-between px-4`}>
          {currentSet === 1 && (
            <button onClick={handlePrevious} className="text-8xl text-[#C59D37] font-light bg-transparent  py-2 px-4 rounded-md ">
              &lt;
            </button>
          )}
          {currentSet === 0 && (
            <button onClick={handleNext} className="text-8xl text-[#C59D37] font-light bg-transparent  py-2 px-4 rounded-md ">
              &gt;
            </button>
          )}
        </div>
